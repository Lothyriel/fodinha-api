//! Reusable Fodinha trick-taking engine.
//!
//! This module owns the rules shared by Classic and Power. It deliberately has
//! no knowledge of power cards, mana, mercenaries, or actor infrastructure.
...
use crate::services::{
    GameInfoDetailsDto, GameInfoDto, GameStageDto, PlayerInfoDetailsDto, PlayerInfoDto,
};
...
impl Game {
    ...
    pub fn get_game_info(&self, player_id: &PlayerId) -> GameInfoDto {
        ...
        GameInfoDto {
            ...
            game: GameInfoDetailsDto::FodinhaClassic,
        }
    }
...
mod tests {
    ...
    fn next_set_clears_bids_for_eliminated_players() {
        ...
        let next_set =
            game.new_set_for_game(std::slice::from_ref(&player2), DealingMode::Increasing, 2);
